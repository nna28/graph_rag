...
- **Lấy lân cận + rerank**: dựng `ego_graph` bán kính `depth` (alias `d`) quanh các neo, duyệt tối đa `neighbor_candidate_multiplier × neighbor_top_k` cạnh, rồi rerank các chuỗi cạnh bằng cosine giữa embedding câu hỏi và embedding chuỗi cạnh, giữ `neighbor_top_k` tốt nhất.
- **Đường đi multi-hop + rerank**: nếu có ≥2 neo, tìm đường đi đơn giản trong đồ thị vô hướng tạm với `cutoff=max_hops`, duyệt tối đa `path_candidate_multiplier × top_k_paths` đường; mỗi cạnh hiển thị chiều (`-[rel]->` hoặc `<-[rel]-`). Rerank các đường theo embedding câu hỏi, giữ `top_k_paths`.
...
- `neighbor_top_k`, `neighbor_candidate_multiplier`: số cạnh lân cận giữ sau rerank và hệ số mở rộng trước khi cắt (20, 3).
- `max_hops`: số cạnh tối đa trên một đường multi-hop (3).
- `top_k_paths`, `path_candidate_multiplier`: số đường multi-hop giữ sau rerank và hệ số mở rộng (5, 3).
